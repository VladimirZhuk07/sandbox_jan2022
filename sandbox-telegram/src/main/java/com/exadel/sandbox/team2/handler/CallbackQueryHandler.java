  ...
  public SendMessage handleSendMessage(Update update, User user) {
    ...
    switch (user.getTelegramState()){
      ...
      case IS_CONFERENCE_HALL_NEED -> sendMessage = telegramService.isConferenceHallNeed(chatId, "Should there be conference hall?\nIf not, then please press Next",
              new String[][]{{"☑️Yes"},{"⬅ ️Back","Next ➡️"}}, new String[][]{{"Yes"},{"Back","Next"}}, data);
      case IS_NEXT_TO_WINDOWS_NEED_BE -> sendMessage = telegramService.isNextToWindowNeedBe(chatId, "Should it be next to window?\nIf no, then please press No.\nIf does not matter then press Next",
              new String[][]{{"☑️Yes","❌ No"},{"⬅ ️Back","Next ➡️"}}, new String[][]{{"Yes","No"},{"Back","Next"}}, data);
      ...
      case ONE_DAY_SELECT_DATE, CONTINUOUS_SELECT_DATE -> sendMessage = utils.getSendMessage(chatId, "Please write start date of your booking in form of `2022-03-10`", new String[][]{{"⬅ ️Back"}}, new String[][]{{"Back"}});
      case RECURRING_SELECT_WEEK_DAY -> sendMessage = utils.getSendMessage(chatId, "Please write all weekdays you want to book in the format `MONDAY,TUESDAY,WEDNESDAY`", new String[][]{{"⬅ ️Back"}}, new String[][]{{"Back"}});
      ...
      case REPORT -> sendMessage = telegramService.checkUserRole(chatId, "Please select report type.",
              new String[][]{{"Users", "Employees"}, {"City", "Single Office"}, {"Offices", "Floor"}, {"⬅ ️Back"}},
              new String[][]{{"USER_REPORT", "ALL_USER_REPORT"}, {"CITY_REPORT", "SINGLE_OFFICE_REPORT"}, {"OFFICES_REPORT", "FLOOR_REPORT"}, {"Back"}}, user);
      case USER_REPORT_DEFINE_BOOKING_FROM, ALL_OFFICE_REPORT_DEFINE_BOOKING_FROM -> sendMessage = utils.getSendMessage(chatId, "Please enter `booking from` date in the form of `2022-03-21`", new String[][]{{"⬅ ️Back"}}, new String[][]{{"Back"}});
      case ALL_USER_REPORT_DEFINE_CREATE_DATE_FROM -> sendMessage = utils.getSendMessage(chatId, "Please enter user`s `created date from` in the form of `2022-03-21`", new String[][]{{"⬅ ️Back"}}, new String[][]{{"Back"}});
      case CITY_REPORT_DEFINE_ID -> sendMessage = utils.getSendMessage(chatId, "Please enter city name in uppercase", new String[][]{{"⬅ ️Back"}}, new String[][]{{"Back"}});
